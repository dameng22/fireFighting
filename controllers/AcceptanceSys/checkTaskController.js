 ...
 */
app.controller('checkTaskController', ['$scope','acceptance_http','exp_tool','all_dic','myself_alert','$timeout','$state','$base64','$stateParams',
	function($scope,acceptance_http,exp_tool,all_dic,myself_alert,$timeout,$state,$base64,$stateParams){
	//时分日历
...
//	];	
	...
	$scope.get_list=function(){
		...
		if($scope.selected == 0){ //完成清单
			...
			param = {pageNum:page_num,pageSize:page_size,nameAndCode:$scope.search_key,taskTypeId:task_type,runflag:runflags}
	...
	$scope.start_check = function(type){
		...
		acceptance_http.start_check_task(params,function(result){
	...
	function scrollUnit(){
		$("#unit").mCustomScrollbar({
			...
			callbacks:{
		        onTotalScroll:function(){
			        if(limits1&&(page_num1<total_page1)){
			        	limits1 = false;
				        $scope.get_unit();
			    	}
		        },
	...
	$scope.start_task = function(){
		...
		var task_area = angular.copy($scope.area_id);
		if(exp_tool.is_chinese(task_area)){
			task_area = null
		}
	...
	//任务类型
	$scope.task_select = all_dic.taskList
	//新增任务初始化
	function init_task(){
		$scope.add_task = {
			  "beginTime": null, //开始时间
