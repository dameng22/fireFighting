 ...
 */
app.controller('fireAlarmController', ['$scope','$location','acceptance_http','myself_alert','all_dic','dic_http','$timeout','new_fire','$rootScope','$filter','$stateParams','$base64',
	function($scope,$location,acceptance_http,myself_alert,all_dic,dic_http,$timeout,new_fire,$rootScope,$filter,$stateParams,$base64){
	//监控器跳转
	$scope.go_monitor = function(){
    ...
    $scope.get_list = function(type){
		...
		acceptance_http.get_fire_alarm({customerId:$base64.decode($stateParams.unit),pageNum:page_num,pageSize:page_size,codeOrName:$scope.search_key,terminalStatusId:$scope.do_list,processStatusId:$scope.undo_list,relayIds:$scope.right_unit},function(result){
			...
			if($rootScope.selectFirst && $scope.fire_list.length>0){
				$scope.get_ids($scope.fire_list[0]);
				$rootScope.selectFirst = false;
			}
			limits = true;
	...
	$scope.deal_alarm = function(type){
		...
		if(type == 'wait'){
			...
			if(type == 'true'){
				$rootScope.confirm_type = "真实火警";
			}else if(type == 'fake'){
				$rootScope.confirm_type = "误报火警";
			}else if(type == 'test'){
				$rootScope.confirm_type = "测试火警";
			}
			$rootScope.fire_now = true;
		}
	};	
