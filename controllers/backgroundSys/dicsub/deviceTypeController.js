 ...
 */
app.controller('deviceTypeController', ['$scope','dic_http','$timeout','myself_alert','$filter','$base64','$stateParams',
	...
	var page_num = 0;
  	...
  	$scope.type_list = [];
  	$scope.undo = false;
	dic_http.get_manufacturer({customerId:$base64.decode($stateParams.unit),isdisable:false,typeId:0},function(result){
		$scope.manu_list = result;
	})
	//列表
	$scope.get_list=function(){
		page_num = page_num+1;
		dic_http.get_device_types({customerId:$base64.decode($stateParams.unit),isdisable:$scope.undo,pageNum:page_num,pageSize:page_size,deviceTypeId:0},function(result){
