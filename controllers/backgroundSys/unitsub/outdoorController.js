 ...
 */
app.controller('outdoorController', ['$scope','acceptance_http','$state','$stateParams','myself_alert','$rootScope',
	...
	$scope.get_list=function(){
		acceptance_http.get_unit_info_outdoor({customerSiteId:$stateParams.unit_id,pageNum:$scope.currentPage,pageSize:1},function(result){
	...
	$scope.save_data_btn=function(){
		...
		acceptance_http.edit_unit_info_outdoor($scope.info[0],function(result){
	...
	$scope.delete_data_btn=function(){
		...
		$rootScope.delete_func = function(){
			acceptance_http.del_unit_info_outdoor({id:$scope.info[0].id},function(result){
