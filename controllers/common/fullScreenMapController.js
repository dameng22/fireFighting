 ...
 */
app.controller('fullScreenMapController', ['$scope','common_http','$interval','$compile','acceptance_http','all_dic','dic_http','$base64','$stateParams','$rootScope','$state',
	function($scope,common_http,$interval,$compile,acceptance_http,all_dic,dic_http,$base64,$stateParams,$rootScope,$state){
    ...
    $scope.init = function (state) {  
		...
		if(all_status.length == 2 || all_status.length == 0){//3
			flag = true;
		}
        ...
        common_http.get_customer_map({isAll:flag,status:all_status,name:$scope.search_key},function(res){
			...
			angular.forEach(res, function (site, i) {
				if(site){
					...
					if($scope.online == false && $scope.offline == false){
						var myIcon = new BMap.Icon(("images/icon_fire_k.png"), new BMap.Size(25, 33));
						label.setStyle({
							display: 'none',
			 			});
					}
					if($scope.online == false && $scope.offline == false && $scope.fire == false){
						var myIcon = new BMap.Icon(("images/icon_fire_k.png"), new BMap.Size(25, 33));
						label.setStyle({
							display: 'none',
			 			});
					...
					if($scope.online == true || $scope.offline == true){
					...
					if($scope.fire == true && label.z.Xp.backgroundColor == "#FF313B"){
						marker.addEventListener("mouseover", function(){
							label.setStyle({
								display: "block",
				 			});
					...
					if($scope.offline == false && marker.z.uj.imageUrl == "images/icon_offline.png"){
						label.setStyle({
							display: 'none',
		 				});
	               		markerClusterer.clearOverlays(marker);
	               	} else if($scope.online == false && marker.z.uj.imageUrl == "images/icon_online.png"){
	               		label.setStyle({
							display: 'none',
		 				});
	               		markerClusterer.clearOverlays(marker);
	               		
	               	} else if($scope.fire == false && marker.z.uj.imageUrl == "images/icon_fire.png"){
	               		markerClusterer.clearOverlays(marker);
	               		map.removeOverlay(marker);
	               	} else if($scope.online == false && $scope.offline == false){
	               		//var myIcon = new BMap.Icon("images/icon_fire.png", new BMap.Size(25, 33));
	               		label.setStyle({
							display: 'none',
	 					});
            ...
            //markerClusterer.addMarkers(markerList);
            map.panTo(new BMap.Point(113.269616,23.15995));   //1分钟后移动到广州
            //数量
